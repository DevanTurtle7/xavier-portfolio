...
class MediaDisplay extends Component {
  ...
  render() {
    ...
    let mediaDisplay = <div>Empty carousel</div>;
    ...
    if (content.length > 0) {
      ...
      if (type === 'image') {
        mediaDisplay = (
          <img src={url} className='admin-media-display' alt='art' />
        );
      } else if (type === 'video') {
        mediaDisplay = (
          <video controls className='admin-media-display'>
            <source src={url} />
          </video>
        );
      }
    ...
    return (
      <Col xs={12} sm={6} md={4} lg={3} xl={2}>
        <Card className='mb-4'>
          <CardBody>
            <Row className='mx-auto'>{mediaDisplay}</Row>
          </CardBody>
          <CardFooter>
            <EditButton
              data={data}
